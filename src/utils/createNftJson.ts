...
const nftCollection = require('../constants/nft-collection');
const { decryptGenInfo } = require('../services/nft-gen/nft-gen.lib');
...
function createPetJsonFile(path: string, gen: string, idNft: string | number) {
    ...
    // console.log("pet", pet);
    const idCollection = pet.petId.toString().padStart(3, '0');

    let template = require('../../nfts/template.json');
    const collect = nftCollection[idCollection];
